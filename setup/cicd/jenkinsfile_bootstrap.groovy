...
def bootstrap_roll_back(){
    ...
    try {
        ...
        //Credential of user
       withCredentials([usernamePassword(credentialsId: 'vinhtx3-gitlab', usernameVariable: 'username',
    ...
    try {
        ...
        if (env.sourceBranchCommitID != env.targetBranchCommitID) {
            ...
            if (currentBuild.result == "SUCCESS") {
                updateGitlabCommitStatus name: "build", state: 'success'
                buildIcon = ":white_check_mark:"
                buildResultStr += "Build Success. Deploy version ${env.project_version} result .Staging Environment IP: <b><a href='http://${env.STAGING_IP}:${env.STAGING_PORT}'>${env.STAGING_IP}:${env.STAGING_PORT}</a></b>"
            } else if (currentBuild.result == "ABORTED") {
                updateGitlabCommitStatus name: "build", state: 'canceled'
                buildIcon = ":warning:"
                buildResultStr = "Build Canceled."
...
def bootstrapDeployToPreRelease() {
    ...
    try {
        ...
        updateGitlabCommitStatus name: "deploy to pre production ", state: 'success'
        //theem vao do 
        if (currentBuild.result != "SUCCESS" && currentBuild.result != "ABORTED") {
                currentBuild.result = "FAILURE"
            }
            ...
            if (currentBuild.result == "SUCCESS") {
                ...
                updateGitlabCommitStatus name: "deploy to pre production ", state: 'canceled'
                buildIcon = ":warning:"
                buildResultStr = "Deploy Canceled."
            } else if (currentBuild.result == "FAILURE") {
                updateGitlabCommitStatus name: "deploy to pre production ", state: 'failed'
                buildIcon = ":x:"
                buildResultStr = "Deploy Fail."
            }
            // Cũ như thế này đây
...
def isOpenMergeRequest(gitlabMergeRequestIid) {
    withCredentials([
        usernamePassword(credentialsId: 'vinhtx3-gitlab', usernameVariable: 'username', passwordVariable: 'password'),
...
return [
    ...
    bootstrapDeployToPreRelease: this.&bootstrapDeployToPreRelease
]
